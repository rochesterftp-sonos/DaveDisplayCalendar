...
5. **Run the app**:
   ...
   On first launch, a settings window will prompt for the number of tenants and per-tenant details. These are saved to `settings.json` in the project root. If you have an `accounts.txt` file in the repo root, use **Import accounts.txt** to populate the form automatically. Each tenant row has a **Login** button that opens the device login in a browser and stores a per-tenant token cache (for example `token_cache_0.json`). The device-code prompt includes the tenant name to make it clear which account to sign in with.
